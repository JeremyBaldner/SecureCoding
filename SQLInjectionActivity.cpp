...
// runs query to search for user match. Updated to validate input - JB
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    // copy of the sql query string
    std::string copy = sql;
    // index of the first '
    int found1 = sql.find('\'') +1;
    // index of the second '
    int found2 = sql.find('\'', found1);

    // If the 'or' operator is found after the name string -> copy = invalid syntax -JB
    if ((sql.find("or", found2) != -1) || (sql.find("OR", found2) != -1))
    {
        copy = "PossibleInjection";
    }
    ...
    char* error_message;
    if (sqlite3_exec(db, copy.c_str(), callback, &records, &error_message) != SQLITE_OK)
    {
