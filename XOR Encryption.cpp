// XOR Encryption.cpp : This file contains the 'main' function. Program execution begins and ends there.
...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
    ...
    std::string fail = "Encypt/Decrypt failed";

    // assert that our input data is good
    if ((key_length < 1) || (source_length < 1))
    {
    ...
    std::bitset<8> key_to_encode(key);

    // loop through the source string char by char
    for (size_t i = 0; i < source_length; ++i)
    { 
        // transform each character based on an xor of the key modded constrained to key length using a mod
        std::bitset<8> letter_to_encode(source[i]);
        std::bitset<8> encoded_value(letter_to_encode ^ key_to_encode);
        output[i] = char(encoded_value.to_ullong());
    }

    // our output length must equal our source length
    if (output.length() == source_length);
    {
...
std::string read_file(const std::string& filename)
{
    ...
    std::string failed_to_open = "unable to open file.";

    // create an input stream
    std::ifstream f(filename);

    // if the file does not open, output error
    if (!f)
    {
    ...
    else 
    {
        ...
        ss << f.rdbuf();            // gather data
        if (!ss.str().empty())      // check for empty file
        {
            file_text = ss.str();  // put into a string if not empty
        }
    }
...
{   // declare file
    ...
    fileOut << "Student name: " << student_name << std::endl;
    fileOut << "Todays date: " << date.tm_year + 1900 << "-" << date.tm_mon + 1 << "-" << date.tm_mday << std::endl;
    fileOut << "Key: " << key << std::endl;
...
std::string getKey()
{
    ...
    // create an 8 character key of random 0s and 1s
    for (int i = 0; i < 8; i++)
    {
        int randomInt = rand() % 2;
        randomKey.append(std::to_string(randomInt));
    }

    // if the random key is all zeros or ones - change it
    if (randomKey.compare("00000000") == 0 || (randomKey.compare("11111111") == 0))
    {
        randomKey = "10101010";
    }
